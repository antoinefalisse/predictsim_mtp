...
int F_generic(const T** arg, T** res) {
	...
	OpenSim::Body* pelvis;
	pelvis = new OpenSim::Body("pelvis", 8.74290024825516454143, Vec3(-0.06827780017111793887, 0.00000000000000000000, 0.00000000000000000000), Inertia(0.07631571244974365287, 0.06466049177405323867, 0.04298326605875640166, 0., 0., 0.));
	model->addBody(pelvis);

	OpenSim::Body* femur_r;
	femur_r = new OpenSim::Body("femur_r", 6.90508723521445055837, Vec3(0.00000000000000000000, -0.17046665777369882089, 0.00000000000000000000), Inertia(0.09994992560112499203, 0.02620046593427548948, 0.10539902535383757765, 0., 0., 0.));
	model->addBody(femur_r);

	OpenSim::Body* tibia_r;
	tibia_r = new OpenSim::Body("tibia_r", 2.75233953217339077923, Vec3(0.00000000000000000000, -0.18048889444253254921, 0.00000000000000000000), Inertia(0.03496743033606343476, 0.00353837092686356235, 0.03545308909073099229, 0., 0., 0.));
	model->addBody(tibia_r);

	OpenSim::Body* talus_r;
	talus_r = new OpenSim::Body("talus_r", 0.07423707436745492771, Vec3(0.00000000000000000000, 0.00000000000000000000, 0.00000000000000000000), Inertia(0.00062007093081916384, 0.00062007093081916384, 0.00062007093081916384, 0., 0., 0.));
	model->addBody(talus_r);

	OpenSim::Body* calcn_r;
	calcn_r = new OpenSim::Body("calcn_r", 0.92796342959318633969, Vec3(0.09139243779317623995, 0.02741773133795287129, 0.00000000000000000000), Inertia(0.00086809930314682936, 0.00241827663019473843, 0.00254229081635857232, 0., 0., 0.));
	model->addBody(calcn_r);

	OpenSim::Body* toes_r;
	toes_r = new OpenSim::Body("toes_r", 0.16079750307990733904, Vec3(0.03162178347643897908, 0.00548354626759057443, -0.01599367661380584477), Inertia(0.00006200709308191638, 0.00012401418616383276, 0.00062007093081916384, 0., 0., 0.));
	model->addBody(toes_r);

	OpenSim::Body* femur_l;
	femur_l = new OpenSim::Body("femur_l", 6.90508723521445055837, Vec3(0.00000000000000000000, -0.17046665777369882089, 0.00000000000000000000), Inertia(0.09994992560112499203, 0.02620046593427548948, 0.10539902535383757765, 0., 0., 0.));
	model->addBody(femur_l);

	OpenSim::Body* tibia_l;
	tibia_l = new OpenSim::Body("tibia_l", 2.75233953217339077923, Vec3(0.00000000000000000000, -0.18048889444253254921, 0.00000000000000000000), Inertia(0.03496743033606343476, 0.00353837092686356235, 0.03545308909073099229, 0., 0., 0.));
	model->addBody(tibia_l);

	OpenSim::Body* talus_l;
	talus_l = new OpenSim::Body("talus_l", 0.07423707436745492771, Vec3(0.00000000000000000000, 0.00000000000000000000, 0.00000000000000000000), Inertia(0.00062007093081916384, 0.00062007093081916384, 0.00062007093081916384, 0., 0., 0.));
	model->addBody(talus_l);

	OpenSim::Body* calcn_l;
	calcn_l = new OpenSim::Body("calcn_l", 0.92796342959318633969, Vec3(0.09139243779317623995, 0.02741773133795287129, 0.00000000000000000000), Inertia(0.00086809930314682936, 0.00241827663019473843, 0.00254229081635857232, 0., 0., 0.));
	model->addBody(calcn_l);

	OpenSim::Body* toes_l;
	toes_l = new OpenSim::Body("toes_l", 0.16079750307990733904, Vec3(0.03162178347643897908, 0.00548354626759057443, 0.01599367661380584477), Inertia(0.00006200709308191638, 0.00012401418616383276, 0.00062007093081916384, 0., 0., 0.));
	...
	humerus_r = new OpenSim::Body("humerus_r", 1.50886853651852126390, Vec3(0.00000000000000000000, -0.16903343413648269644, 0.00000000000000000000), Inertia(0.00936367267046308259, 0.00323017705298663579, 0.01051042079677209570, 0., 0., 0.));
	model->addBody(humerus_r);

	OpenSim::Body* ulna_r;
	ulna_r = new OpenSim::Body("ulna_r", 0.45099022678228867234, Vec3(0.00000000000000000000, -0.11842869140885815826, 0.00000000000000000000), Inertia(0.00212307571783212695, 0.00044296448130325952, 0.00230298524017374245, 0., 0., 0.));
	model->addBody(ulna_r);

	OpenSim::Body* radius_r;
	radius_r = new OpenSim::Body("radius_r", 0.45099022678228867234, Vec3(0.00000000000000000000, -0.11842869140885815826, 0.00000000000000000000), Inertia(0.00212307571783212695, 0.00044296448130325952, 0.00230298524017374245, 0., 0., 0.));
	model->addBody(radius_r);

	OpenSim::Body* hand_r;
	hand_r = new OpenSim::Body("hand_r", 0.33963461523110627382, Vec3(0.00000000000000000000, -0.06691061390986266511, 0.00000000000000000000), Inertia(0.00063935973676781141, 0.00039207373992375882, 0.00096047314716240725, 0., 0., 0.));
	model->addBody(hand_r);

	OpenSim::Body* humerus_l;
	humerus_l = new OpenSim::Body("humerus_l", 1.50886853651852126390, Vec3(0.00000000000000000000, -0.16903343413648269644, 0.00000000000000000000), Inertia(0.00936367267046308259, 0.00323017705298663579, 0.01051042079677209570, 0., 0., 0.));
	model->addBody(humerus_l);

	OpenSim::Body* ulna_l;
	ulna_l = new OpenSim::Body("ulna_l", 0.45099022678228867234, Vec3(0.00000000000000000000, -0.11842869140885815826, 0.00000000000000000000), Inertia(0.00212307571783212695, 0.00044296448130325952, 0.00230298524017374245, 0., 0., 0.));
	model->addBody(ulna_l);

	OpenSim::Body* radius_l;
	radius_l = new OpenSim::Body("radius_l", 0.45099022678228867234, Vec3(0.00000000000000000000, -0.11842869140885815826, 0.00000000000000000000), Inertia(0.00212307571783212695, 0.00044296448130325952, 0.00230298524017374245, 0., 0., 0.));
	model->addBody(radius_l);

	OpenSim::Body* hand_l;
	hand_l = new OpenSim::Body("hand_l", 0.33963461523110627382, Vec3(0.00000000000000000000, -0.06691061390986266511, 0.00000000000000000000), Inertia(0.00063935973676781141, 0.00039207373992375882, 0.00096047314716240725, 0., 0., 0.));
