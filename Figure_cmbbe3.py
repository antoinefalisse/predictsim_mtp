...
# %% Load experimental data Lundgren2007
pathLundgren = os.path.join(pathMain, 'OpenSimModel', 'Lundgren2007')
from scipy.interpolate import interp1d
import pandas as pd
tOut = np.linspace(0, 100, 101)
dataInterp = np.zeros([101, 5])
for i in range(5):
    pathLundgrenCurvei = os.path.join(pathLundgren, 'Curve{}.csv'.format(i))
    curve_df = pd.read_csv(pathLundgrenCurvei)
    set_interp = interp1d(curve_df['x'], -curve_df['y'])
    dataInterp[:,i] = set_interp(tOut)
    
fig = plt.figure()
ax = plt.gca()
lineObjects = ax.plot(tOut,dataInterp[:,0],
                      tOut,dataInterp[:,1],
                      tOut,dataInterp[:,2],
                      tOut,dataInterp[:,3],
                      tOut,dataInterp[:,4])

dataInterp_mean = np.mean(dataInterp, axis=1)
dataInterp_std = np.std(dataInterp, axis=1)
    ...
    if i < NJointsToPlot:
        for c_no_mtp, case in enumerate(cases_no_mtp):
            ...
            if case == case_4exp:
                ...
                if i == 1:
                    ax.fill_between(tOut,
                            dataInterp_mean + 1*dataInterp_std,
                            dataInterp_mean - 1*dataInterp_std,
