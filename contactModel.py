...
class smoothSphereHalfSpaceForce:
    ...
    def getContactForce(self, locSphere_inB, posB_inG, lVelB_inG, aVelB_inG,
        ...
        # It looks like CasADi does not recognize the cross product of numpy.
        # To be able to use this function with and without CasADi inputs, let's
        # just decompose the cross product.
#        v = lVelB_inG.T +  np.cross(aVelB_inG.T, 
#                                    locSphere_inG - 
...
#                                    posB_inG.T)        
        ...
        vnormal = v[0, 0]*self.normal[0, 0] + v[0, 1]*self.normal[0, 1] + v[0, 2]*self.normal[0, 2]
        vtangent =  v - vnormal * self.normal
        indentationVel = -vnormal
        # Stiffness force  
        k = 0.5 * (self.stiffness) ** (2/3)
        fH = (4/3) * k * np.sqrt(self.radius * k) * ((np.sqrt(indentation * indentation + eps)) ** (3/2))
        # Dissipation force
        fHd = fH * (1 + 1.5 * self.dissipation * indentationVel)
        fn = ((0.5 * 
               np.tanh(bv * (indentationVel + 1 / (1.5 * self.dissipation))) + 
               0.5 + eps2) * (0.5 * np.tanh(bd * indentation) + 0.5 + eps2) * fHd)
        force = fn * self.normal
        # Friction force
        aux = (vtangent[0, 0]) ** 2 + (vtangent[0, 1]) ** 2 + (vtangent[0, 2]) ** 2 + eps
        vslip = aux ** (0.5)
...
#        crossProduct1 = term1[0, 2]*term2[0, 0] - term1[0, 0]*term2[0, 2]
#        crossProduct2 = term1[0, 0]*term2[0, 1] - term1[0, 1]*term2[0, 0]           
#        crossProduct = ca.horzcat(crossProduct0, crossProduct1, crossProduct2)      
#        v = lVelB_inG.T + crossProduct
        vnormal = v[0, 0]*self.normal[0, 0] + v[0, 1]*self.normal[0, 1] + v[0, 2]*self.normal[0, 2]
        vtangent =  v - vnormal * self.normal
        indentationVel = -vnormal
        # Stiffness force  
