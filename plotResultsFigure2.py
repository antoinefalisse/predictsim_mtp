...
kinematic_ylim_ub = [40, 20, 20, 20, 30, 30, 50]
kinematic_ylim_lb = [-30, -20, -20, -80, -30, -30, -10]
...
jointsToPlot_no_mtp = ['hip_flexion_r', 'hip_adduction_r', 'hip_rotation_r', 
               'knee_angle_r',  'ankle_angle_r', 
               'subtalar_angle_r']
...
fig, axs = plt.subplots(7, 5, sharex=False)    
...
for i, ax in enumerate(axs[:, 0]):
    ...
    if i < NJointsToPlot:
        for idxc, case in enumerate(cases_mtp):
            ax.plot(optimaltrajectories[case]['GC_percent'],
                    optimaltrajectories[case]['coordinate_values'][idxJointsToPlot[i]:idxJointsToPlot[i]+1, :].T, c=colors[idxc], linewidth=3, linestyle=linestyles[idxc], label='case_' + case + '_mtp')
        ...
        #                 optimaltrajectories_no_mtp[case]['coordinate_values'][idxJointsToPlot_no_mtp[count]:idxJointsToPlot_no_mtp[count]+1, :].T, c='orange', linestyle='dashed', linewidth=3, label='case_' + case + '_no_mtp')   
...
muscleTitles = ['glut_med1_r', 'Gluteus medius', 'glut_med3_r', 'Gluteus minimus', 
               ...
               'Tibialis posterior', 'Flexor digitorum', 'Flexor hallucis', 'Tibialis anterior', 'per_brev_r',
               'per_long_r', 'per_tert_r', 'Extensor digitorum', 'Extensor hallucis', 'ercspn_r', 
               'intobl_r', 'extobl_r',
...
musclesToPlot = ['vas_med_r', 
                 'med_gas_r', 'soleus_r', 'tib_ant_r',
                 'tib_post_r', 'flex_dig_r', 'flex_hal_r', 'ext_dig_r', 'ext_hal_r']
...
for i, ax in enumerate(axs[1:4,2:].flat):
    color_mtp=iter(plt.cm.rainbow(np.linspace(0,1,len(cases_mtp))))  
    color_no_mtp=iter(plt.cm.rainbow(np.linspace(0,1,len(cases_no_mtp))))  
    if i < NMusclesToPlot:
        for idxc, case in enumerate(cases_mtp):
            ax.plot(optimaltrajectories[case]['GC_percent'],
                    optimaltrajectories[case]['muscle_activations'][idxMusclesToPlot[i]:idxMusclesToPlot[i]+1, :].T, c=colors[idxc], linewidth=3, linestyle=linestyles[idxc], label='case_' + case + '_mtp')  
        ...
        # for case in cases_no_mtp:
        #     ax.plot(optimaltrajectories_no_mtp[case]['GC_percent'],
        #             optimaltrajectories_no_mtp[case]['muscle_activations'][idxMusclesToPlot[i]:idxMusclesToPlot[i]+1, :].T, c='orange', linestyle='dashed', linewidth=3, label='case_' + case + '_no_mtp')            
    ...
    # plt.legend(handles, labels, loc='upper right')
# plt.setp(axs[-1, :], xlabel='Gait cycle (%)')
plt.setp(axs[1:4, 2], ylabel='(-)')
fig.align_ylabels()

# %% Kinetics
kinetic_ylim_ub = [70,30,30,70,30,30,10]
kinetic_ylim_lb = [-70,-90,-20,-50,-110,-30,-30]
...
for i, ax in enumerate(axs[:,1]):
    ...
    if i < NJointsToPlot:
        for idxc, case in enumerate(cases_mtp):
            ax.plot(optimaltrajectories[case]['GC_percent'],
                    optimaltrajectories[case]['joint_torques'][idxJointsToPlot[i]:idxJointsToPlot[i]+1, :].T, c=colors[idxc], linewidth=3, linestyle=linestyles[idxc], label='case_' + case + '_mtp')  
        ...
        #     for case in cases_no_mtp:
        #         ax.plot(optimaltrajectories_no_mtp[case]['GC_percent'],
        #                 optimaltrajectories_no_mtp[case]['joint_torques'][idxJointsToPlot_no_mtp[count]:idxJointsToPlot_no_mtp[count]+1, :].T, c='orange', linestyle='dashed', linewidth=3, label='case_' + case + '_no_mtp')            
...
for i, ax in enumerate(axs[0,2:]):
    ...
    if i < NGRFToPlot:
        for idxc, case in enumerate(cases_mtp):
            ax.plot(optimaltrajectories[case]['GC_percent'],
                    optimaltrajectories[case]['GRF'][idxGRFToPlot[i]:idxGRFToPlot[i]+1, :].T, c=colors[idxc], linewidth=3, linestyle=linestyles[idxc], label='case_' + case + '_mtp')  
        ...
        # for case in cases_no_mtp:
        #     ax.plot(optimaltrajectories_no_mtp[case]['GC_percent'],
        #             optimaltrajectories_no_mtp[case]['GRF'][idxGRFToPlot[i]:idxGRFToPlot[i]+1, :].T, c='orange', linestyle='dashed', linewidth=3, label='case_' + case + '_no_mtp')  
        ...
        # plt.legend(handles, labels, loc='upper right')
# plt.setp(axs[-1, :], xlabel='Gait cycle (%)')
plt.setp(axs[0, 2], ylabel='(N)')
fig.align_ylabels()


# %%
for ax in (axs[4:7,2:5].flat):
